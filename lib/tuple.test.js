...
describe('Tuple', () => {
  ...
  describe('cross product', () => {
    test('two vectors', () => {
      const v1 = Tuple.vector(1, 2, 3);
      const v2 = Tuple.vector(2, 3, 4);
      expect(v1.crossProduct(v2)).toEqual(Tuple.vector(-1, 2, -1));
      expect(v2.crossProduct(v1)).toEqual(Tuple.vector(1, -2, 1));
    });

    test('only vectors', () => {
      const v1 = Tuple.vector(1, 2, 3);
      const p1 = Tuple.point(1, 2, -3);
      expect(() => v1.crossProduct(p1)).toThrowError(RtError);
      expect(() => p1.crossProduct(v1)).toThrowError(RtError);
    });
  });

  describe('Reflection', () => {
    test('Reflecting a vector approaching at 45°', () => {
      const v = Tuple.vector(1, -1, 0);
      const n = Tuple.vector(0, 1, 0);
      const r = v.reflect(n);
      expect(r).toEqual(Tuple.vector(1, 1, 0));
    });
    test('Reflecting a vector off a slanted surface', () => {
      const v = Tuple.vector(0, -1, 0);
      const n = Tuple.vector(Math.SQRT2 / 2, Math.SQRT2 / 2, 0);
      const r = v.reflect(n);
      expect(r).shallowEpsilonEquals(Tuple.vector(1, 0, 0));
    });
  });
});
