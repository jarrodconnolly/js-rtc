...
describe('Checker Pattern', () => {
  ...
  test('Checkers uv map', () => {
    const p = new Checker(Colour.white(), Colour.black(), true);
    const s = new Sphere();
    expect(p.patternAt(s, Tuple.point(0, 0, 0))).toEqual(Colour.black());
    expect(p.patternAt(s, Tuple.point(1, 0, 0))).toEqual(Colour.white());
    expect(p.patternAt(s, Tuple.point(2, 0, 0))).toEqual(Colour.white());
    expect(p.patternAt(s, Tuple.point(1, 1, 0))).toEqual(Colour.black());
    expect(p.patternAt(s, Tuple.point(2, 1, 0))).toEqual(Colour.white());
  });
});
