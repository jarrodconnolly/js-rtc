...
describe('Camera', () => {
  ...
  test('Render a single pixel', () => {
    const w = defaultWorld();
    const c = new Camera(20, 20, Math.PI / 2);
    const from = Tuple.point(0, 0, -5);
    const to = Tuple.point(0, 0, 0);
    const up = Tuple.vector(0, 1, 0);
    c.transform = Transform.viewTransform(from, to, up);
    const p = c.renderPixel(w, 10, 10);
    expect(p).shallowEpsilonEquals(new Colour(0.22935, 0.28669, 0.17201), 5);
  });
});
